...
* Installeer de Woots PowerShell module, indien nog niet gedaan, met ``Install-Module -Name Woots`` in een PowerShell-venster (al dan niet met administratorrechten naar behoefte).
...
``whatif`` is een schakelaar die bepaalt of Write-Phase wijzigingen in Woots aanbrengt ten behoeve van testen en debuggen. Waarde 0 betekent: schrijf wijzigingen, 1 betekent: doe alsof en schrijf geen wijzigingen.

``do_remove_instructors`` is een schakelaar. Waarde 0 betekent: laat staan. Waarde 1 betekent: verwijder docenten uit programma's indien ze niet meer voorkomen in de door ReadPhase aangemaakte lijst.

``gridview`` is een schakelaar. Waarde 1 betekent: ReadPhase toont interactief het resultaat in een GridView-venster. Deze schakelaar is handig voor het beoordelen van het resultaat van ReadPhase, maar kan problemen opleveren als WootsSync wordt uitgevoerd als geplande taak zonder toezicht (unattended), omdat er dan geen gebruiker is om het GridView-venster te sluiten. Waarde 0 betekent dat ReadPhase geen GridView-venster toont.

``onbekendvak_overslaan`` is een schakelaar. Waarde 1 betekent dat ReadPhase vakken in de uitvoer overslaat, die niet konden worden gekoppeld aan een bekende vakomschrijving. 

``do_remove_classes`` is een schakelaar. Waarde 1 betekent : verwijder niet-gekoppelde klassen uit programma's. Waarde 0 betekent: laat ze staan.

=== WootsSync-UserTable.txt

In de WootsSync-UserTable.txt kan een vertalingstabel worden gedefinieerd voor gebruikersnamen. De linkerkolom bevat de gebruikersnaam zoals deze wordt vastgesteld op basis van de gegevens in Magister. De rechterkolom bevat de gebruikersnaam zoals deze in Woots voorkomt. De scheiding tussen de kolommen is een is-gelijkteken (=). Deze vertalingstabel wordt gebruikt in WritePhase bij het koppelen van docenten aan programma's.
...
* toevoegen docenten die ook beheerder zijn. Voorlopig moeten deze docenten zichzelf toevoegen.
* leerjaar toevoegen aan programma's
* documentatie voor UPN vertaaltabel :-) 